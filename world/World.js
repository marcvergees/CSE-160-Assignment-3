...
function addActionsForHTMLUI() {
    ...
    // Clear existing options and populate from maps object
    mapSelect.innerHTML = '';
    ...
    mapSelect.addEventListener("change", function () {
        g_map = maps[this.value];
        g_camera.reset();
        console.log("Switched to map: " + this.value);
        this.blur();
    });
}
...
function keydown(ev) {
    // Prevent game keys from interacting with focused UI elements
    const gameKeys = [32, 87, 65, 83, 68, 81, 69, 37, 38, 39, 40, 80, 27, 9];
    if (gameKeys.includes(ev.keyCode)) {
        ev.preventDefault();
    }
